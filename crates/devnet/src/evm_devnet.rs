...
use alloy::{
    node_bindings::{Anvil, AnvilInstance}, primitives::{Address, U256}, providers::{ext::AnvilApi, DynProvider, Provider}, sol
};

use crate::{
    evm_devnet::SimpleERC20::SimpleERC20Instance, get_new_temp_dir, token_indexerd::TokenIndexerInstance, RiftDevnetCache
};
...
pub struct EthDevnet {
    ...
    pub cbbtc_contract: SimpleERC20Instance<DynProvider>,
...
impl EthDevnet {
    ...
    pub async fn setup(
        ...
        let token_indexer = if using_token_indexer {
            Some(TokenIndexerInstance::new(anvil.endpoint_url().to_string().as_str(), anvil.ws_endpoint_url().to_string().as_str(), false, anvil.chain_id()).await?)
        } else {
            None
        };
    ...
    pub async fn mint_cbbtc(&self, address: Address, amount: U256) -> Result<()> {
        self.cbbtc_contract.mint(address, amount).send().await?.get_receipt().await?;
        Ok(())
    }
...
async fn deploy_cbbtc(provider: DynProvider, devnet_cache: Option<Arc<RiftDevnetCache>>) -> Result<SimpleERC20Instance<DynProvider>> {
    if let Some(cache) = devnet_cache {
        // no need to deploy, just create the instance from the cache
        let cbbtc_contract = SimpleERC20Instance::new(CBBTC_ADDRESS.parse().unwrap(), provider);
        return Ok(cbbtc_contract);
    }

    provider.anvil_set_code(CBBTC_ADDRESS.parse().unwrap(), CBBTC_BYTECODE.parse().unwrap()).await?;

    let cbbtc_contract = SimpleERC20Instance::new(CBBTC_ADDRESS.parse().unwrap(), provider);

    cbbtc_contract.setConfig("CBBTC".to_string(), "CBBTC".to_string(), 9).send().await?.get_receipt().await?;
