...
async fn test_indexer_client(
    ...
    let devnet = RiftDevnet::builder().using_token_indexer(true).build().await.unwrap().0;
    // now mint some token

    info!("Minting CB-BTC to funded address");
    devnet.ethereum.mint_cbbtc(devnet.ethereum.funded_address, U256::from(1_000_000)).await.unwrap();

    info!("Transferring CB-BTC to to address");
    println!("to.ethereum_address: {}", to.ethereum_address);
    devnet.ethereum.cbbtc_contract.transfer(to.ethereum_address, U256::from(100)).send().await.unwrap().get_receipt().await.unwrap();


    // print the current block number
    let block_number = devnet.ethereum.funded_provider.get_block_number().await.unwrap();
    info!("Current block number: {}", block_number);


    // print hte balance of the to address
    let balance = devnet.ethereum.cbbtc_contract.balanceOf(to.ethereum_address).call().await.unwrap();
    info!("Balance of to address: {}", balance);

    // Get the token indexer URL
    let indexer_url = devnet.ethereum.token_indexer
        .as_ref()
    ...
    let indexer_client = TokenIndexerClient::new(&indexer_url)
    ...
    for i in 0..max_retries {
        ...
        if let Ok(transfer_response) = result {
            if !transfer_response.transfers.is_empty() {
                info!("Found {} transfers after {} retries", transfer_response.pagination.total, i);
    ...
    assert!(!transfers.transfers.is_empty(), "Expected at least one transfer");
    ...
    assert_eq!(latest_transfer.to, to.ethereum_address, "Transfer 'to' address mismatch");
    assert_eq!(latest_transfer.from, devnet.ethereum.funded_address, "Transfer 'from' address mismatch");
    
    // Validate the amount - we sent 100 tokens
    let expected_amount = U256::from(100);
    let actual_amount = U256::from_str_radix(&latest_transfer.amount, 10)
        .expect("Failed to parse transfer amount");
    
    assert_eq!(actual_amount, expected_amount, "Transfer amount mismatch: expected {}, got {}", expected_amount, actual_amount);
    ...
    info!("  - Block: {} (hash: {:?})", latest_transfer.block_number, latest_transfer.block_hash);
}
